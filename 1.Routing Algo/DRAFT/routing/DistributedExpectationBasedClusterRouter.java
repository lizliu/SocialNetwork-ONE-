/*
 * DEBTC router for The One Simulator
 *
...
public class DistributedExpectationBasedClusterRouter extends ActiveRouter {
	

	public static final String EXPECTATION_SETTINGS = "DistributedExpectationBasedClusterRouter";
	...
	public DistributedExpectationBasedClusterRouter(Settings s) {
		super(s);
	...
	public DistributedExpectationBasedClusterRouter(DistributedExpectationBasedClusterRouter proto) {
		super(proto);
	...
	@Override
	public DistributedExpectationBasedClusterRouter replicate() {
		return new DistributedExpectationBasedClusterRouter(this);
	}
	...
	protected boolean commumesWithHostDirectly(DistributedExpectationBasedClusterRouter othRouter, DTNHost destination) {
	...
	protected boolean commumesWithHostIndirectlyAndNotThis(DistributedExpectationBasedClusterRouter othRouter, DTNHost destination) {
		HashSet<DTNHost> othersBranch = othRouter.getBranchSetOnlyNotThis(this.getHost());
	...
	public HashSet<DTNHost> getBranchSetOnlyNotThis(DTNHost aHost) {
		...
		while (it.hasNext()) {
			...
			if(!branch.contains(aHost) && (pairs.getKey() != aHost)) {
	...
	public void processPromotionToLocalCommunity() {	
		...
		while (it.hasNext()) {
			...
			if(count >= (this.joinCoefficient*this.currBaseline)) {
				...
				this.localCommunity.put(pairs.getKey(),  ((DistributedExpectationBasedClusterRouter) pairs.getKey().getRouter()).getLocalCommunitySet());
			}
	...
	private Tuple<Message, Connection> tryOtherMessages() {	
		...
		for (Connection con : getConnections()) {
			...
			DistributedExpectationBasedClusterRouter othRouter = (DistributedExpectationBasedClusterRouter)other.getRouter();
