				...
				implements RoutingDecisionEngine, CommunityDetectionEngine
{
   ...
   /* public boolean shouldSendMessageToHost(Message m, DTNHost otherHost,
			Map<String, Integer> conntimes, DTNHost myhost)
	{	
		if(m.getTo() == otherHost) return true;
		
		DistributedBubbleRap de = getOtherDecisionEngine(otherHost);
		boolean myhave = connHistory.keySet().contains(m.getTo()); 
		boolean peerhave = de.connHistory.keySet().contains(m.getTo());
		
		//int threshold = 100;//mit
		//int threshold = 200;//infor05
		int threshold = 500;//cam
		
		
		Integer nrofCopies = (Integer) m.getProperty("SPRAYANDWAIT_NS"
				+ "." + "copies");
		int mycount=0, peercount=0;
		...
		if (nrofCopies >= 1) {	
			...
			{
				if ((peercount + mycount) != 0) {
					myutility = alpha * mycount / (peercount + mycount) + beta
							* mymember / (peermember + mymember);
					peerutility = alpha * peercount / (peercount + mycount)
							+ beta * peermember / (peermember + mymember);
				} else {
					myutility = beta * mymember / (peermember + mymember);
					peerutility = beta * peermember / (peermember + mymember);
				}
				if (myutility < peerutility)
					return true;
				else
					return false;
			}
		...
		}/*else if( nrofCopies <= 3 && nrofCopies >= 1){
	...
	public boolean othershouldDeleteSentMessage(Message m, DTNHost otherHost)
	{
		// DiBuBB allows a node to remove a message once it's forwarded it into the
		// local community of the destination
		DistributedBubbleRap de = this.getOtherDecisionEngine(otherHost);
    ...
    {//System.out.println( from.toString() + " -- " + to.toString() );
	 ...
	 public String FindBestConn(Message m, DTNHost otherHost, DTNHost myHost){
		 if(m.getTo() == otherHost) return "dest"; 
			
			DTNHost dest = m.getTo();
			DistributedBubbleRap de = getOtherDecisionEngine(otherHost);
			DistributedBubbleRap my = getOtherDecisionEngine(myHost);
		//	System.out.println(my.getGlobalCentrality());
		//	System.out.println(de.getGlobalCentrality());
			boolean peerInCommunity = de.commumesWithHost(dest);
			boolean meInCommunity = my.commumesWithHost(dest);
			...
			{		
				...
				{//System.out.println("1");
					String s = String.valueOf(de.getLocalCentrality() - this.getLocalCentrality());
					//System.out.println("1");
					return "local_"+s;
				}
				else
				{//int a[] = new int[10];a[-1]=0;
					return "";
				}
			}	
			else if(de.getGlobalCentrality() > this.getGlobalCentrality())
			{
				String s = String.valueOf(de.getGlobalCentrality() - this.getGlobalCentrality());
				return "global_"+s;
			}
